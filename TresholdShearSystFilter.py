...
class TresholdSSFilter(VTKPythonAlgorithmBase):
    ...
    def __init__(self):
        ...
        self._filename = (
            "/Users/mik/Documents/dpmm/codes/foxtrot/fx_fft/fftbuild/tests/evp.mat"
        )
        self._save = False
    ...
    @smproperty.stringvector(
        name="FileName",
        default_values="/Users/mik/Documents/dpmm/codes/foxtrot/fx_fft/fftbuild/tests/evp.mat",
    )
    @smdomain.filelist()
    ...
    def RequestData(self, request, inInfoVec, outInfo):
        ...
        gamma_by_grain, mean_field, _ = edt.compute_mean_field(
            grains_index,
            field_data,
            field_name="Gamma",
            vx_size=vx_size,
        ...
        if datas:
            ...
            if self._save:
                ...
                comments = " Shear systems : {0} \n".format(
                    np.transpose(edt.LIST_TYPES_INDEXED)
                )
                comments = comments + " treshold = {} ; TIME = {}\n".format(
                    self._shear_treshold, time
                )
                comments = (
                    comments
                    + " number of grains = {} ; size of data = {}\n".format(
                        real_indx_grains.shape, syst_activated.shape
                    )
                )

                header = (
                    comments
                    + " Written by {}\n# grain_number    shear_syst_1st     shear_sys_2nd    ....".format(
                        FILTER_NAME
                    )
                )
                np.savetxt(
                    "systems_activated_by_index_t{}.rpt".format(t),
                    syst_activated,
                    fmt="%i",
                    header=header,
                )
            ...
            output.FieldData.append(field_data.FieldData["TIME"], "TIME")
            print(
                " Do nothing ... : no grains found with treshold set {}".format(
                    self._shear_treshold
                )
            )

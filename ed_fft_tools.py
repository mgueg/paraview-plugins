...
This module is a collection of tools to do post-treatment based for FFT result, more precisely for EVP-FFT result
...
beta_Cij = np.array([[100,70,70,0,0,0],
                [70,100,70,0,0,0],
                [70,70,100,0,0,0],
                [0,0,0,36,0,0],
                [0,0,0,0,36,0],
...
def angles_in_deg(ea_field):
    ...
    return np.all(np.abs(ea_field)<360) & np.all(np.abs(ea_field)>0.) & np.any(np.abs(ea_field)>np.pi)
...
def load_data(field_data, data_name, dim_in=None):
    ...
    if dim_in:
        dim_size = tuple(d-1 for d in dim_in)
    array = point_data.PointData[data_name]
...
def compute_theta_direction(angles, load_direction="y"):
    """
    Compute theta function in of direction loading considering euler angles in radian
    by default it is currently defined as the angle between the c axis of the HCP lattice and the Y axis
    
    """
    assert load_direction=="y",\
...
def compute_young_modulus(Cij,h,k,l):
    ...
    E = 1/(S[0,0] - (2 * (S[0,0] - S[0,1]) - S[3,3]) \
        * (h**2*k**2+k**2*l**2+l**2*h**2)/(h**2+k**2+l**2)**2)/10**9 
        
    return E
    
def center_of_mass(index_voxels):
    """
        Return center of mass of grains
    """
    return np.array([[np.average(component) for component \
        in np.nonzero(index_voxels==indx)] for  indx in  np.unique(index_voxels)])
...
def mat_from_euler_angles(phi):
    ...
    Args:
        phi = 3 angles d'Euler en radians et pas degre
    ...
    ..note:: 
        ...
        with:
            - tsi i component of t vector from sample frame
            - tci i component of t vector from crystalline frame
    '''
...
#     phi2 = np.radians(phi[2])
    ...
    P = np.array([[c1*c2 - s1*s2*cG, -c1*s2 - s1*c2*cG, s1*sG],
                  [s1*c2 + c1*s2*cG, -s1*s2 + c1*c2*cG, -c1*sG],
                  [s2 * sG,          c2 * sG,           cG]])
    return P.transpose()


def compute_volume(grain_index_field,vx_size=(1.,1.,1.)):
    """
...
def find_grains_border(grains_index_field):
    ...
    mask = ((grad_index[:,0]!=0.) | (grad_index[:,1]!=0.) | (grad_index[:,2]!=0.))
    mask = np.asarray(mask, dtype=np.uint8)
    #output.PointData.append(mask, "labels")
    return mask
   

def compute_mean_field(grain_index_field,field_data,field_name,vx_size=(1.,1.,1.),
                        weighted=True, compute_std_dev=False):
    """
    ...
    Args:
        ...
        vx_size=(1.,1.,1.):     the voxel size
        weighted=True:          whether or not the mean and stddev is weighted by grain volume 
        compute_std_dev=False:          whether we compute standard deviation for `field_name`
...
def change_orientation_for_beta_phase(field_data, beta_orientation):
    ...
    #On charge les champs permettant de différencier les phases (=1 si alpha et 2 si beta)
    image_data = load_data(field_data,"ImageData") 
    
    #On charge la colonne qui contient les numéros de grains et on regarde le nombre total de grains
    ...
    #On crée le champ Phases nécessaire au calculateur (=1 si phase alpha et 2 si phase beta)
    phases = np.zeros(grains_index.shape[0]) 
    phases = image_data/255+1
    
    #On charge les champs permettant de différencier les zones (matrice ou grain central)
    in_zone = load_data(load_data,"ImageDatacercle",dim_data) 
    in_data_zone = np.unique(grains[in_zone==255])
    out_data_zone= np.unique(grains[in_zone!=255])
    #On cherche les nodules qui sont coupés par la frontière 
    # entre les deux zones et donc qui sont à la fois dans la matrice et dans le grain
    boarder = np.intersect1d(out_data_zone,in_data_zone) 
    print(boarder)
    
    #On charge les angles d'Euler de la phase alpha
    ea_alpha = load_data(data,"EulerAnglesalpha",dim_data)
    #Lorsque la matrice est aléatoire, on charge également 
    # les angles d'euler de la phase beta qui n'a donc plus une orientation unique 
    ea_beta = load_data(data,"EulerAnglesbeta",dim_data) 
    ...
    ea_field[mbeta2] = beta_orientation[0] #Ligne à changer lorsque l'orientation de la phase beta 'du grain central est modifiée
    ...
    #ipf_beta2=load_data(data,"IPFColorbeta001z",dim_data) #Ligne à changer lorsque l'orientation de la phase beta 'du grain central est modifiée. Doit concorder avec la ligne 53
    #
    ...
    # On associe les nodules aux frontières à la zone à laquelle ils appartiennent 
    # le plus en comptant le nombre de voxels de chacun de ces grains dans chaque zone.
    for grain in boarder: 
        ...
        if l1>l2: #Si il y a plus de voxels dans le grain central
            ...
            in_zone[m1i] = 255 #La zone occupée par ce nodule est integrée au grain central
